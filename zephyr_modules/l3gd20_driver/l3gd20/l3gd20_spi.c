...
{
  const size_t num_regs = (to - from) + 1;
                     ...
                     uint8_t* value)
{
  uint8_t write_cmd =
      reg & (~L3GD20_SPI_READ_BIT &
             ~L3GD20_SPI_MS_BIT);  // Write, don't increment address
  const struct spi_buf tx_buf[2] = { { .buf = &write_cmd, .len = 1 },
                                     { .buf = value, .len = 1 } };
  const struct spi_buf_set tx = { .buffers = tx_buf, .count = 2 };

  const struct spi_buf rx_buf = { .buf = NULL, .len = 2 };
                      ...
                      enum l3gd20_reg to, uint8_t* values)
{
  ...
  uint8_t write_cmd = from | (~L3GD20_SPI_READ_BIT &
                              L3GD20_SPI_MS_BIT);  // Write, increment address
  const struct spi_buf tx_buf[2] = { { .buf = &write_cmd, .len = 1 },
                                     { .buf = values, .len = num_regs } };
  const struct spi_buf_set tx = { .buffers = tx_buf, .count = 2 };

  const struct spi_buf rx_buf = { .buf = NULL, .len = num_regs + 1 };
  const struct spi_buf_set rx = { .buffers = &rx_buf, .count = 1 };
