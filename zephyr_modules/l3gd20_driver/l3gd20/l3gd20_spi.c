...
int l3gd20_read_reg(const struct device* spi_bus, const struct spi_config* spi_conf, enum l3gd20_reg reg, uint8_t* value)
{
  uint8_t read_cmd = reg | (L3GD20_SPI_READ_BIT & ~L3GD20_SPI_MS_BIT);  // Read, don't increment address
...
int l3gd20_write_reg(const struct device* spi_bus, const struct spi_config* spi_conf, enum l3gd20_reg reg, uint8_t value)
{
